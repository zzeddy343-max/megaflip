-- Force binary settlement RPCs to use public.trade_status values for trades
-- and public.transaction_status values for ledger rows. Some deployed
-- databases still have an older settle_trade body that writes
-- "completed" into trades.status, which is invalid for public.trade_status.
