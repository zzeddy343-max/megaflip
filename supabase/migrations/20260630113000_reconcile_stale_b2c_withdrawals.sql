-- Reconcile hanging B2C withdrawals safely.
-- If a withdrawal is still pending/processing after the approval window and
-- no successful Safaricom callback exists, mark it failed so the reserved
-- balance is refunded. If a late success callback arrives after a refund,
-- apply_transaction deducts the money again.
...
begin
  ...
  if _tx.kind = 'withdraw'
    ...
    if _tx.account_type = 'real' then
      update public.profiles
      set balance_usd = greatest(balance_usd - _tx.amount_usd, 0)
      where id = _tx.user_id;
    else
      update public.profiles
      set demo_balance_usd = greatest(demo_balance_usd - _tx.amount_usd, 0)
      where id = _tx.user_id;
    end if;
  end if;
