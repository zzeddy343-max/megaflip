-- Restore the previously deployed B2C payout behavior.
-- Daraja acceptance releases the reserved withdrawal immediately. A later
-- failure/timeout callback still refunds it through apply_transaction.
...
-- Repair requests accepted by Daraja before this migration but left in
-- processing by the regression.
do $$
declare
  _row record;
begin
  for _row in
    select t.id, pr.response_payload
    from public.transactions t
    join public.payment_requests pr on pr.transaction_id = t.id
    where t.kind = 'withdraw'
      and t.method = 'mpesa'
